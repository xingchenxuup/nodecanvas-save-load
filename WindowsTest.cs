...
#region 序列化数组
    ...
    WindowsTest()
    {
        this.titleContent = new GUIContent("保存黑板");
    }
...
#region 序列化数组
    ...
    {
        EditorGUILayout.BeginHorizontal();
        if (GUILayout.Button("Json黑板"))
        {
            blackboardType = BlackboardType.Json;
        }
        if (GUILayout.Button("黑板实例"))
        {
            blackboardType = BlackboardType.Blackboard;
        }
        EditorGUILayout.EndHorizontal();
        
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("GraphOwner Type",GUILayout.Width(150));
        index = EditorGUILayout.Popup(index,options,GUILayout.Width(50));
        EditorGUILayout.EndHorizontal();
